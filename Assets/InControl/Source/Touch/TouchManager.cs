...
#pragma warning restore 414
		...
		Vector3 ConvertScreenToWorldPoint( Vector2 point )
		{
			...
			{
				return touchCamera.ScreenToWorldPoint( new Vector3( point.x, point.y, touchCamera.nearClipPlane ) );
			}
		...
		Vector3 ConvertViewToWorldPoint( Vector2 point )
		{
			if (TouchCameraIsValid())
			{
				return touchCamera.ViewportToWorldPoint( new Vector3( point.x, point.y, touchCamera.nearClipPlane ) );
			}
		...
		Vector3 ConvertScreenToViewPoint( Vector2 point )
		{
			if (TouchCameraIsValid())
			{
				return touchCamera.ScreenToViewportPoint( new Vector3( point.x, point.y, touchCamera.nearClipPlane ) );
			}
